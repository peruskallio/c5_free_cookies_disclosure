    ...
    public static function on_page_view()
    {
        ...
        if (!$p->isAdminArea() && !$p->isError() && !COOKIES_ALLOWED) {
            ...
            if (strlen(COOKIES_DISCLOSURE_COLOR_PROFILE) > 0) {
                $v->addHeaderItem($html->css('cookies_disclosure_' . COOKIES_DISCLOSURE_COLOR_PROFILE . '.css',
                    'free_cookies_disclosure'));
                $v->addHeaderItem('<!--[if lte IE 8]>' . $html->css('cookies_disclosure_' . COOKIES_DISCLOSURE_COLOR_PROFILE . '_ie.css',
                        'free_cookies_disclosure') . '<![endif]-->');
            }
            ...
            // FIXME how should I load this without Loader?
            ...
            Events::extend('on_render_complete', 'FreeCookiesDisclosureDisclosureRenderer', 'on_render_complete',
                $file);
        }
    }
